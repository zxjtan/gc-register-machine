## chenny garbage collection with register machine

#### Different types of pointer

we defined the pointer types by pair the different type of tags with the index. Below are all the types available within the program.
...
We can simply get the type of pointer with
...
If we wanna check the pointer matched specify type, we can use
...
For every valid pointers, we can wrap and unwrap the pointer by
...
we have a list of primitive functions, each primitive function is a pair of name and number of required parameters.
...
With the primitive lists, we can set up the environment by using setup_environment function
...
we have some helper functions with controller sequence which include make controller sequence, check controller sequence and so on.
...
There are some basic helper functions used inside the project
...
Anohter function is help to store the parse tree contents into memory
...
There are some general functions which applicable for all basic machines.We will categorise them into different components.
...
##### Getting and setter for the machine
...
We need to assemble the controller sequence list machine readable format
...
We need a simulator to construct register machine code. There are general different forms of getter and setter API.
...
There is a main function which handle making different types of register machine code.
...
Followed by different individuals  make functions
...
### Register machine code for basic operations

We need to convert basic operations such as pair, list, into register machine code.
...
The correponding individual components will be a control sequence.
...
##### Evaluate application operand loop
...
##### Evaluate application accumlate arguments
...
##### Evaluate application last argument
...
##### Evaluate application accumulated last argument
...
### 4.1 to register machine code

We need to convert some of the functions in section 4.1 of the textbook to register machine code.
...
### Constuct complete machine

With all the things we mentioned previously, we still need operations list for the machine.But before that, let's add helper function that can support memory management.With the helper function, we then will complete the machine by combine operations list, controller sequence list together.
...
For the operations list, we have different types of operation list.
...
##### Garbage collection operation list
...
we combine various operation list and pass it to the machine.
